...
void main() {
  group('AuthenticatedApiInterceptor', () {
    test('adds the bearer token to protected requests', () async {
      ...
      expect(
        adapter.requests.single.headers[authorizationHeader],
        'Bearer access-1',
      );
    });
    ...
    test('does not add bearer token to refresh requests', () async {
      ...
      await dio.post<void>(
        '/auth/refresh',
        data: const <String, Object>{'refreshToken': 'refresh-1'},
      );

      expect(adapter.requests.single.headers[authorizationHeader], isNull);
    });
    ...
    test('refreshes once and retries a failed request', () async {
      ...
      final adapter = _RecordingAdapter(
        responseFor: (request) {
          final authorization = request.headers[authorizationHeader];
          ...
          return const _AdapterReply(statusCode: 200, body: '{"ok":true}');
        },
      );
      final dio = _createDio(adapter);
      ...
      expect(
        adapter.requests.last.headers[authorizationHeader],
        'Bearer access-2',
      );
    });
...
class _AdapterReply {
  const _AdapterReply({required this.statusCode, this.body = '{}'});
