...
class AuthenticatedApiInterceptor extends Interceptor {
  ...
  }) : this._(dio, readAccessToken, refreshSession);
  ...
  static const String _retryAttemptedKey = 'authenticated_api_retry_attempted';
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    final existingAuthorization = options.headers[_authorizationHeader];

    if (existingAuthorization is String &&
        existingAuthorization.trim().isNotEmpty) {
      options.extra[_sentAccessTokenKey] = _extractBearerToken(
  ...
  Future<void> _retry({
    ...
    request.headers[_authorizationHeader] = 'Bearer $accessToken';
